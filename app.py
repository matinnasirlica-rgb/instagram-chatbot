...
def send_instagram_message(recipient_id: str, message: str):
    ...
    # Token bitibsə yenilə və yenidən cəhd et
    if "error" in result and result["error"].get("code") == 190:
        ...
        response = requests.post(url, json=payload, headers=headers)
        result = response.json()
        print(f"📤 Yenidən göndərildi: {result}")
...
def manual_refresh():
    """Əl ilə token yenilə: https://instagram-chatbot-oixw.onrender.com/refresh"""
    refresh_instagram_token()
